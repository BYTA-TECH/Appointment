...
import com.bytatech.ayoos.appointment.config.KafkaProperties;
...
class AppointmentKafkaResourceIT {
    ...
    void setup() {
        ...
        AppointmentKafkaResource kafkaResource = new AppointmentKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/appointment-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/appointment-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
